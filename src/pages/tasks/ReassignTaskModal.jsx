...
function ReassignTaskModal({ task, onClose,}) {
  ...
  const handleReassign = async () => {
    ...
    try {
      await axios.patch(`assign/reassign/${taskId}`, { newAssignee: selectedStaff }, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      toast.success("Task reassigned!");
      onClose();
    } catch (err) {
      toast.error("Failed to reassign task", err);
      console.error(err);
    }
  };
    ...
    <div className="fixed inset-0 bg-[#000000c6] bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-[400px]">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={handleReassign} className="bg-purple-600 text-white px-4 py-1 rounded">
            Reassign
          </button>
        </div>
      </div>
