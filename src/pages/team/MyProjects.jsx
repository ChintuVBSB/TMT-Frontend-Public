...
const MyProjects = () => {
  ...
  const handleCompleteMilestone = async (projectId, milestoneId) => {
    try {
      ...
      const progress = calculateMilestoneProgress(updatedProject?.milestones || []);
      if (progress === 100) {
    ...
    <div className="bg-white relative rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition">
            ...
            {project.milestones.map((m, idx) => (
              ...
              >
                <Checkbox
                  checked={m.completed}
                  onChange={() => {
                    if (!m.completed) handleCompleteMilestone(project._id, m._id);
                  }}
                />
                <div className="flex flex-col">
                  <p
                    className={`font-medium text-sm ${
                      m.completed ? "text-green-800 line-through" : "text-gray-800"
                    }`}
                  >
                    {m.title}
                  </p>
      ...
      {Array.isArray(project.staffs) && (
        <div className="text-sm text-indigo-700 mb-3 flex items-center gap-2">
          ...
          {
            (() => {
              const leadMember = project.staffs.find(
                (s) => String(s._id) === String(project.lead)
              );
              return leadMember ? leadMember.name : "Not Assigned";
            })()
          }
        </div>
      )}
      ...
      <div className="max-w-6xl mt-12 mx-auto px-4 pt-10 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {(activeTab === "active" ? activeProjects : completedProjects).length > 0 ? (
            (activeTab === "active" ? activeProjects : completedProjects).map((p) => (
